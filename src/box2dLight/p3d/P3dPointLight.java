...
public class P3dPointLight extends P3dPositionalLight {
	 ...
	 */
	public P3dPointLight(P3dRayHandler lightManager, int rays) {
	 ...
	 */
	public P3dPointLight(P3dRayHandler lightManager, int rays, Color color, float distance, float x, float y) {
	...
	public void dynamicShadowRender () {
		for (int i = 0; i < activeShadows; i++) {
			...
			m.render(lightHandler.lightShader, GL20.GL_TRIANGLE_STRIP);
		}
	}
		...
		for (Fixture fixture : affectedFixtures) {
			...
			if (type == Type.Polygon || type == Type.Chain) {
				...
				float angle = (MathUtils.PI2 - 2f * a) /
						P3dRayHandler.CIRCLE_APPROX_POINTS;
				for (int k = 0; k < P3dRayHandler.CIRCLE_APPROX_POINTS; k++) {
					tmpStart.set(center).add(tmpVec);
			...
			if (activeShadows >= dynamicShadowMeshes.size) {
				mesh = new Mesh(
						...
						P3dRayHandler.MAX_SHADOW_VERTICES, 0,
